...
Notex - RPKG, HM3CustomMaps (https://github.com/Notexe/HM3CustomMaps), Statue of Liberty prims and materials
OrfeaZ - ZHM Mod
printable_models - Statue of Liberty model from https://free3d.com/3d-model/statue-of-liberty-v1--175955.html
Thanks to everyone at the Glacier 2 Modding discord including Kevin Rudd, Anthony Fuller, Invalid, Notex, Sieni, Atampy26, kercyx, 2kpr, volvomodus, HMBM47, and everyone for helping me along the way and encouraging me to continue.

## To Do List:

**Dartmoor**: Fix starting location, add more scenery obstacles to block access to the sides of the mansion, ...  
**Dubai**: Remove end credits, change weather to a clear day, replace train with plane, change target name to Eugene Cobb ...  
**Siberia**: Switch to Custom mission, ...  
**Marrakesh**: Remove soldiers from parking garage and tunnel, add security guards to tunnel, remove key requirement for consulate car exit, add Hajun with escape condition, change starting location to start of tunnel, make all doors out of the tunnel and parking garage permanently locked, ...  
**New York**: Add objective to steal documents from the vault, add enforcer status to all guards, ...  
**Sapienza**: Change target model to Thomas Cross, change objective from killing Thomas Cross to confronting and then kidnapping him, ...  
**Colorado**: Remove all starting locations except for house hacker start, change house start to be in basement with suit, make house start available offline, change objectives to speak with targets, make targets die after talking with them, ...  
**Mumbai**: Add Ether memory wiping antidote holding device, add objective to obtain Ether memory wiping antidote, change objective from killing scientist to planting bomb, replace Mole's behavior with the prisoner's from Marrakesh, replace the Mole's model with an Ether scientist from Sapienza...  
**Sgail**: Add kill switch item to a stash spot, ...  
**Berlin**: Make daytime, remove all entrances other than the radio tower and all exits, add exit in forest, add objective to disable machine at the top of the radio tower, make radio tower door unlockable, add normal security guards at radio tower, make elite guards appear after disabling the radio tower, ...
